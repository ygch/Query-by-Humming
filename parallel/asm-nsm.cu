...
float* Asm(float *query,int qline,int seq_num,float *all_seq,int all_length,int *seq_length,int *accu_length,float r)
{
    ...
    */
    ...
    t1=clock();
    _asm<<<seq_num,THREAD_NUM>>>(qline,all_seq_gpu,seq_length_gpu,
        ...
        int *left_bound_gpu,int *right_bound_gpu,float *small_gpu)
{
    ...
    for(int i=3;i<qline;i++)
    {
        ...
        {
            float v1=valid_pos(left_bound_gpu[i-1-(j-start)],right_bound_gpu[i-1-(j-start)],x_gpu[out_start+prev_start+j],y_gpu[out_start+prev_start+j],1); 
            float v2=valid_pos(left_bound_gpu[i-1-(j-start)],right_bound_gpu[i-1-(j-start)],x_gpu[out_start+prev_start+j+1],y_gpu[out_start+prev_start+j+1],2); 
    ...
    for(int i=qline;i<=seq_length;i++)
    {
        ...
        {
            float v1=valid_pos(left_bound_gpu[qline-1-j],right_bound_gpu[qline-1-j],x_gpu[out_start+prev_start+j],y_gpu[out_start+prev_start+j],1); 
            float v2=valid_pos(left_bound_gpu[qline-1-j],right_bound_gpu[qline-1-j],x_gpu[out_start+prev_start+j+1],y_gpu[out_start+prev_start+j+1],2); 
